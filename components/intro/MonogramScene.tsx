...
export function MonogramScene({ onComplete, fadeTargetRef }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Gate 1: renderer construction. Nothing has been created yet, so
      // failure here needs no cleanup beyond the fail-open callback.
      ...
      // Single disposal routine shared by the success cleanup path and the
      // gate-2 failure path below, so a resource added later can't be freed
      ...
      // Gate 2: everything else. Any throw here (chunk geometry, animation
      // building, etc.) must dispose whatever was already created — the
      // renderer, any appended canvas, any geometries pushed so far — before
      // failing open.
      try {
        ...
        materials.push(faceInk, faceCopper, side);
        ...
        // behind. This is an opacity fade, not motion, so power2.inOut (not
        // power3.out) is correct here — it doesn't overshoot or bounce.
        timeline.to(
